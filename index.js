...
const dbCreateClaimSlot   = (cid, gid)  => db.prepare('INSERT OR REPLACE INTO claim_slots(channel_id,guild_id,claimed_by,claimed_at,expires_at) VALUES(?,?,NULL,NULL,NULL)').run(cid, gid);
const dbGetClaimSlot      = (cid)      => db.prepare('SELECT * FROM claim_slots WHERE channel_id=?').get(cid);
const dbAllClaimSlots     = (gid)      => db.prepare('SELECT * FROM claim_slots WHERE guild_id=?').all(gid);
const dbClaimSlot         = (cid, uid, exp) => db.prepare('UPDATE claim_slots SET claimed_by=?, claimed_at=?, expires_at=? WHERE channel_id=?').run(uid, Date.now(), exp, cid);
const dbDeleteClaimSlot   = (cid)      => db.prepare('DELETE FROM claim_slots WHERE channel_id=?').run(cid);
const dbUserHasClaim      = (gid, uid) => !!db.prepare('SELECT 1 FROM claim_slots WHERE guild_id=? AND claimed_by=?').get(gid, uid);
...
async function checkExpiredSlots() {
  ...
  // Check expired claim slots — revert them back to unclaimed
  for (const [, guild] of client.guilds.cache) {
    for (const row of dbAllClaimSlots(guild.id)) {
      ...
      if (ch) {
        // Remove claimer permissions
        await ch.permissionOverwrites.edit(row.claimed_by, {
          [PermissionFlagsBits.SendMessages]: false,
          [PermissionFlagsBits.ViewChannel]: false,
        }).catch(() => {});
        await ch.setName('🎟️-unclaimed-slot').catch(() => {});
        await ch.send('⏰ This slot has expired and is now available to claim again. Use `?claimslot` to claim it!');
      }
      // Reset to unclaimed
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (cmd === '?setclaim') {
    ...
    if (isNaN(num) || num < 1 || num > 50) return message.reply('❌ Usage: `?setclaim <number>` (1–50)');
    ...
    for (let i = 0; i < num; i++) {
      try {
        ...
        const claimMsg = '🎟️ **This slot is available to claim!**' + '\n\n' +
          '> Type `?claimslot` in this channel to claim it for **7 days**.' + '\n' +
          '> You must have the **Member** role to claim.' + '\n' +
          '> Only one claim per member at a time.';
        await ch.send(claimMsg);
        created.push(ch);
      } catch (e) { console.error('Failed to create claim slot:', e); }
    }

    return message.reply(`✅ Created **${created.length}** claimable slot${created.length !== 1 ? 's' : ''}!`);
  }

  // ── ?claimslot ────────────────────────────────────────────────────────────
  if (cmd === '?claimslot') {
    // Must be a claim slot channel
    const claimData = dbGetClaimSlot(channel.id);
    if (!claimData) return;

    // Must have Member role
    if (!member.roles.cache.has(MEMBER_ROLE_ID))
      return message.reply('❌ You must have the **Member** role to claim a slot.');

    // Already claimed
    if (claimData.claimed_by)
      return message.reply('❌ This slot has already been claimed and is now a private slot.');

    // Can't already have a slot or another claim
    if (dbUserHasSlot(guild.id, member.id))
      return message.reply('❌ You already have an active slot.');
    if (dbUserHasWeekend(guild.id, member.id))
      return message.reply('❌ You already have an active weekend slot.');
    if (dbUserHasClaim(guild.id, member.id))
      return message.reply('❌ You have already claimed a slot.');

    const exp = Date.now() + 7 * 24 * 60 * 60 * 1000;
    dbClaimSlot(channel.id, member.id, exp);

    // Give claimer full slot permissions, remove public view
    ...
    // Rename channel
    const safe = member.user.username.toLowerCase().replace(/[^a-z0-9]/g, '');
    await channel.setName(`🎲-${safe}s-slot-7d`).catch(() => {});

    await channel.send(
      `🎲 ${member} has claimed this slot!

` +
      `⏳ Lasts **7 days** — expires <t:${Math.floor(exp / 1000)}:R>.
` +
      `📢 You have **1x \`@here\`** and **1x \`@everyone\`** to use.
` +
      `👥 Use \`?talk @user ...\` to invite people to chat here.`
    );

    // Also save into regular slots table so all other commands work on it
    dbSaveSlot(channel.id, member.id, guild.id, 'free', '🎲', exp);
